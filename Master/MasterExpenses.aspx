<%@ Page Language="VB" AutoEventWireup="false" CodeFile="MasterExpenses.aspx.vb" Inherits="Master_MasterExpenses" %>
                ...
                function gridCommand(sender, args) {
                    if (args.get_commandName() == "BatchEdit") {
                        ...
                        for (var i = 0; i < dataItems.length; i++) {
                            var idCellGUID = dataItems[i].get_cell("ExpensesTextT"); //get reference to the cells you want to validate

                            var idGUID = batchManager.getCellValue(idCellGUID); //retrieve the value from the cell using the BatchEditingManager 

                            if (idGUID == "") {
                                if (idGUID == "") msg += "<%= Me.GridShowdata.MasterTableView.GetColumn("ExpensesTextT").HeaderText%>" + " " + "<%=Sys.TMsg.ไม่สามารถมีค่าว่างเปล่า%>" + "<br/>";
                        ...
                        if (itemCNT >= 0) {
                            msg += "<%= Me.GridShowdata.MasterTableView.GetColumn("ExpensesTextT").HeaderText%>" + " " + "<%=Sys.TMsg.ไม่สามารถมีค่าซ้ำ%>" + "<br/>";
                            validationFails = true;
                        }
                        else {
        ...
        <div>
            <telerik:RadGrid ID="GridShowdata" runat="server" AllowMultiRowEdit="True"
                ...
                <MasterTableView CommandItemDisplay="Top" CommandItemSettings-ShowRefreshButton="false" EditItemStyle-Width="100px"
                    ...
                    EditMode="Batch" EnableNoRecordsTemplate="true" DataKeyNames="ExpensesID">
                    <BatchEditingSettings EditType="Cell"  />
                    <NoRecordsTemplate></NoRecordsTemplate>
                    <ItemStyle Height="28px" />
                    <AlternatingItemStyle Height="28px" />
                    <Columns>
                        <telerik:GridBoundColumn UniqueName="ExpensesTextT" SortExpression="ExpensesTextT" DataField="ExpensesTextT" HeaderText="รายการ" AllowSorting="true" HeaderStyle-Width="100px" ColumnEditorID="txtEditor2" />
                        <telerik:GridBoundColumn UniqueName="ExpensesTextE" SortExpression="ExpensesTextE" DataField="ExpensesTextE" HeaderText="รายการ(อังกฤษ)" AllowSorting="true" HeaderStyle-Width="120px" ColumnEditorID="txtEditor2" />
