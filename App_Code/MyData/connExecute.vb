...
Public Class connExecute
    ...
    Private me_DBType As DatabaseType
...
#Region " Sub New "
    ...
    'Public Sub New(Optional ByVal x_DbType As DatabaseType = DatabaseType.SQLServer)
    ...
    '            cnsql = New SqlConnection(Global.Connection.sqlcon)
    '    End Select
    'End Sub
    'constructure ประโชน์คือไว้กำหนดค่าเริ่มต้นให้กับโปรแกรม
    'Public Sub New(Optional ByVal x_DbType As DatabaseType = DatabaseType.SQLServer)

    'Public Sub New(ByVal x_DbType As DatabaseType)
    '    me_DBType = x_DbType   'เลือกชนิด  ฐานข้อมูล
    '    Select Case me_DBType
    '        Case DatabaseType.Access
    '            Throw New NotImplementedException
    ...
    Public Sub New(Optional xConnectionString$ = "", Optional ByVal x_DbType As DatabaseType = DatabaseType.SQLServer)
        me_DBType = x_DbType   'เลือกชนิด  ฐานข้อมูล
        Select Case me_DBType
            Case DatabaseType.Access
                Throw New NotImplementedException
            ...
            Case DatabaseType.SQLServer
                If xConnectionString = "" Then
                    cnsql = New SqlConnection(Global.Connection.sqlcon)
                Else
                    cnsql = New SqlConnection(xConnectionString)
                End If
        End Select
    End Sub
...
#End Region
    ...
    Public Sub parameter(ByVal Fpara As String, ByVal Fvalue As Object)
        Fpara = Fpara.ToLower '1/2/2018 พี่ต่อเพิ่มให้
    ...
    Public Sub sqlParameter_Value(ByVal xname As String, ByVal xtype2 As String, ByVal Fvalue As Object)
        xname = xname.ToLower '1/2/2018 พี่ต่อเพิ่มให้
    ...
    ''' </code>
    Public Function CreateCommand(ByVal xSql$, Optional ByVal xCon As IDbConnection = Nothing) As System.Data.Common.DbCommand
        cmdsql = MyFactoy.Instance(me_DBType).CreateCommand
        If xCon Is Nothing Then
            cmdsql.Connection = cnsql
        Else
            cmdsql.Connection = xCon
        End If
        If me_DBType = DatabaseType.SQLServer Then
            xSql = sqlStrFormat(xSql)
        End If
        cmdsql.CommandText = xSql
        Return cmdsql
    End Function
    ...
    Public Sub sqlParameter(ByVal xtype As String)
        xtype = xtype.ToLower '1/2/2018 พี่ต่อเพิ่มให้
...
Public Class MyFactoy
    Shared _sqlProvider As DbProviderFactory = Nothing
    Shared _oleProvider As DbProviderFactory = Nothing

    Public Shared Function Instance(ByVal x_DbType As DatabaseType) As DbProviderFactory
        If x_DbType = DatabaseType.SQLServer Then
            Return If(_sqlProvider, DbProviderFactories.GetFactory("System.Data.SqlClient"))
        ElseIf x_DbType = DatabaseType.Access Then
            Return If(_oleProvider, DbProviderFactories.GetFactory("System.Data.OleDb"))
        Else
            Throw New NotImplementedException
        End If
    End Function
